...
As the saying goes, "all roads lead to Rome", there are a couple of ways to get a low-privilege shell. I'll take the path of least resistance.
...
Again, there are other ways to obtain `root`:
...
With a `root` shell, the last step is to get the `root` flag as proof that you have `root`ed the machine.
